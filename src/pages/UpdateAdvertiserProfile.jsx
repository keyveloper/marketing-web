...
import { uploadAdvertiserProfileImage } from '../api/advertiserProfileImageApi.js';
import { uploadAdvertiserProfileInfo, issueAdvertiserProfileDraft } from '../api/advertiserProfileApi.js';
import './ProfileAdvertiser.css';
import './CreateProfileAdvertiser.css';

export default function UpdateAdvertiserProfile({ existingData }) {
  ...
  useEffect(() => {
    if (existingData) {
      console.log('✅ 기존 프로필 데이터로 초기화:', existingData);

      const { profileApiResult, profileImage, backgroundImage } = existingData;

      setFormData({
        name: profileApiResult?.advertiserName || '',
        title: '',
        backgroundImage: null,
        backgroundImagePreview: backgroundImage?.presignedUrl || '',
        profileImage: null,
        profileImagePreview: profileImage?.presignedUrl || '',
        industry: profileApiResult?.serviceInfo || '',
        location: profileApiResult?.locationBrief || '',
        website: '',
        email: '',
        description: profileApiResult?.introduction || '',
      });

      // 기존 draftId 사용
      if (profileApiResult?.userProfileDraftId) {
        setDraftId(profileApiResult.userProfileDraftId);
      }
    }
  }, [existingData]);
        ...
        if (result.success) {
          ...
          if (imageType === 'background') {
            setFormData(prev => ({
              ...prev,
              backgroundImage: null,
              backgroundImagePreview: existingData?.backgroundImage?.presignedUrl || ''
            }));
          } else if (imageType === 'profile') {
            setFormData(prev => ({
              ...prev,
              profileImage: null,
              profileImagePreview: existingData?.profileImage?.presignedUrl || ''
            }));
          }
        }
      } catch (error) {
